#!/usr/bin/env python3
"""
cube_pose_visualizer.py — RViz marker publisher for the detected cube pose.

Subscribes to /cube_pose and /cube_detected and publishes:
  /cube_marker   — a cube-shaped Marker (green = detected, red = lost)
  /cube_axes     — XYZ axis arrows showing cube orientation
"""
...
class CubePoseVisualizer(Node):
    """Publishes RViz markers showing the detected cube pose and orientation axes."""
    ...
    def __init__(self) -> None:
        ...
        """
        super().__init__("cube_pose_visualizer")
...
def main(args=None) -> None:
    """ROS2 entry point — start the cube pose visualizer node."""
    rclpy.init(args=args)
    node = CubePoseVisualizer()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
