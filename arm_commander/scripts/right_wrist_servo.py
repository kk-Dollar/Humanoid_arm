...
"""
right_wrist_servo.py — Right wrist camera visual servoing node.

All logic is in wrist_servo_base.py (WristServoBase / RightWristServoNode).
This file is the ROS 2 entry-point script installed by CMakeLists.txt.

Topics (right arm):
  SUB  /right_wrist_cam/image_raw    (sensor_msgs/Image)     camera frames
  SUB  /right_wrist_cam/camera_info  (sensor_msgs/CameraInfo) intrinsics
  PUB  /right_wrist/servo_correction (geometry_msgs/Twist)   XY corrections (m)
  PUB  /right_wrist/cube_width_m     (std_msgs/Float64)      measured cube width
  PUB  /right_wrist/aligned          (std_msgs/Bool)         convergence flag
  PUB  /right_wrist/debug_image      (sensor_msgs/Image)     annotated debug frame
  SRV  /right_wrist/align            (std_srvs/Trigger)      blocking align call
"""
...
from wrist_servo_base import RightWristServoNode


def main(args: list | None = None) -> None:
    """
    WHAT: Entry point for the right wrist servo process.
